...
export const fetchRealEvents = async (forceRefresh = false): Promise<EsportsEvent[]> => {
  if (!forceRefresh) {
  ...
  try {
        ...
        responseSchema: {
          ...
          items: {
            ...
            properties: {
              ...
              eventType: { type: Type.STRING, enum: ["New Update", "Royal Pass", "New Season", "Special Event"] },
              startDateTime: { type: Type.STRING },
              version: { type: Type.STRING },
              description: { type: Type.STRING },
              highlights: { type: Type.ARRAY, items: { type: Type.STRING } }
            },
            required: ["id", "game", "eventName", "eventType", "startDateTime", "description"]
          }
        }
      },
    });
    ...
    const events: EsportsEvent[] = rawEvents.filter(e => {
      ...
      if (e.eventType === EventType.ROYAL_PASS || e.eventType === EventType.SEASON) {
    ...
    const uniqueEvents = finalData.filter((v, i, a) => 
      a.findIndex(t => t.eventName === v.eventName && t.game === v.game) === i
    );
 ...
 */
export const fetchTacticalBriefing = async (forceRefresh = false): Promise<string> => {
  if (!forceRefresh) {
  ...
  const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
  const prompt = "Provide a concise, 3-sentence 'Situational Report' on the current state of BGMI and PUBG Mobile competitive play and version updates. Use military/tactical terminology.";
  ...
  try {
    ...
    const text = response.text || "Operational data currently classified. Establish secure uplink for briefing.";
    sessionStorage.setItem(BRIEFING_CACHE_KEY, JSON.stringify({ text, timestamp: Date.now() }));
    return text;
  } catch {
